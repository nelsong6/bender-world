...
resource "azurerm_cosmosdb_sql_role_assignment" "container_app_cosmos" {
  ...
  principal_id        = azurerm_container_app.api["bender-api"].identity[0].principal_id
...
resource "azurerm_dns_txt_record" "api_verification" {
  ...
  record {
    value = azurerm_container_app.api["bender-api"].custom_domain_verification_id
  }
}
...
resource "azapi_update_resource" "api_cert_binding" {
  ...
  resource_id = azurerm_container_app.api["bender-api"].id
